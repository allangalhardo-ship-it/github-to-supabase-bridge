...
interface ParsedInsumo extends ImportInsumoData {
  status: 'valid' | 'duplicate' | 'error';
  message?: string;
  existingId?: string;
}

interface ImportInsumosDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  existingInsumos: { id: string; nome: string }[];
}

const ImportInsumosDialog = ({ open, onOpenChange, existingInsumos }: ImportInsumosDialogProps) => {
  const { usuario } = useAuth();
  const { toast } = useToast();
  
  const [step, setStep] = useState<'upload' | 'review' | 'importing'>('upload');
  const [parsedData, setParsedData] = useState<ParsedInsumo[]>([]);
  const [parseErrors, setParseErrors] = useState<string[]>([]);
  const [isImporting, setIsImporting] = useState(false);

  const handleDownloadTemplate = () => {
    downloadTemplate(INSUMOS_TEMPLATE, 'modelo_insumos', 'Insumos');
  ...
  const handleFileUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const { data, errors } = await parseExcelFile<ImportInsumoData>(file, INSUMOS_TEMPLATE);
      
      // Check for duplicates
      const processedData: ParsedInsumo[] = data.map(item => {
        const normalizedName = normalizeString(item.nome);
        const existing = existingInsumos.find(
          i => normalizeString(i.nome) === normalizedName
        );
        ...
        if (isNaN(custo) || custo < 0) {
          return {
            ...
            message: 'Custo unitário inválido',
          };
        }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      toast({ 
        ...
        description: `${validItems.length} insumos importados com sucesso.` 
      });
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            {step === 'importing' && 'Importando insumos...'}
          </DialogDescription>
        </DialogHeader>
                ...
                <TableBody>
                  {parsedData.map((item, index) => (
                    <TableRow key={index} className={item.status !== 'valid' ? 'opacity-60' : ''}>
                      ...
                      <TableCell className="text-right">R$ {parseFloat(item.custo_unitario || '0').toFixed(2)}</TableCell>
                      <TableCell className="text-right">{item.estoque_atual || '0'}</TableCell>
        ...
        {step === 'importing' && (
          <div className="flex flex-col items-center justify-center py-12 gap-4">
            ...
            <p className="text-muted-foreground">Importando insumos...</p>
          </div>
        )}
      </DialogContent>
    </Dialog>
...
export default ImportInsumosDialog;
