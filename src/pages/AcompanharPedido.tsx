...
const STATUS_CONFIG = [
  { key: 'pendente', label: 'Pedido recebido', icon: Clock, color: 'text-amber-500' },
  { key: 'confirmado', label: 'Confirmado', icon: CheckCircle2, color: 'text-blue-500' },
  { key: 'preparando', label: 'Preparando', icon: ChefHat, color: 'text-orange-500' },
  { key: 'pronto', label: 'Pronto', icon: Package, color: 'text-emerald-500' },
  { key: 'entregue', label: 'Entregue', icon: Truck, color: 'text-emerald-600' },
];
  ...
  useEffect(() => {
    ...
    const fetchPedido = async () => {
      ...
      if (error || !data) {
        ...
        setPedido(data as unknown as PedidoTracking);
      }
      setLoading(false);
    };
    ...
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        setPedido(payload.new as unknown as PedidoTracking);
      })
      .subscribe();
  ...
  if (notFound || !pedido) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6 text-center">
        ...
        <h1 className="text-xl font-bold text-gray-700">Pedido não encontrado</h1>
        ...
        {isCancelado ? (
          <div className="bg-red-50 border border-red-200 rounded-xl p-4 text-center">
            ...
            {pedido.motivo_cancelamento && (
              <p className="text-sm text-red-600 mt-1">{pedido.motivo_cancelamento}</p>
            )}
          </div>
        ) : (
                  ...
                  <div key={s.key} className="flex items-start gap-3">
                    <div className="flex flex-col items-center">
                      <div className={cn(
                        ...
                        <Icon className={cn("h-4 w-4", isActive ? s.color : "text-gray-300")} />
                      </div>
                      {i < STATUS_CONFIG.length - 1 && (
                        <div className={cn("w-0.5 h-6", i < statusIndex ? "bg-emerald-300" : "bg-gray-200")} />
                      )}
                    </div>
                    <div className="pt-1">
                      <p className={cn(
          ...
          {itens.map((item: any, i: number) => (
            <div key={i} className="flex justify-between text-sm">
              ...
              <span className="font-medium text-gray-800">
                {formatCurrencyBRL((item.preco_unitario || 0) * (item.quantidade || 1))}
              </span>
            </div>
          ))}
